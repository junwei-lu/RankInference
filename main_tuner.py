...
def f_theta_h2(Y_l, time, edge, L_ij, t, lambda_val, h, n, p, ground_truth, iter_gd, stepsize, a=0.1, b=0.1):
    theta_h = np.zeros(n)  # Assuming 'n' is defined elsewhere as the number of entities
    sq_error = np.zeros((n, iter_gd))
    grad1 = gradient2(Y_l, time, edge, L_ij, theta_h, t, h, lambda_val, n, p)['grad1']
    sum_iter = 0
    while sum_iter < iter_gd:
    ## while np.sum(np.abs(grad1)) > 1e-6 and sum_iter <= iter_gd:
    ## while np.sum(grad1 ** 2) > 1e-6 and sum_iter <= 100:
        ...
        while L_t2(Y_l, time, edge, L_ij, theta_h - m * grad1, t, h, lambda_val, n, p) > \
            ...
            m = b * m
            reduce += 1
        sq_error[:, sum_iter] = (theta_h - ground_truth) ** 2
        sum_iter += 1
...
def parse_argument():
    ...
    parser.add_argument('--iter_gd', type=int, default=100, help='Number of GD iterations in estimation')
    parser.add_argument('--step_size', type=float, default=30, help='Number of step size in GD')
...
def compute_theta_h(i, t, L_ij, p, n, lambda_val, h, seed, w_iter, iter_gd, stepsize):
    ...
    theta_h, sq_error = f_theta_h2(Y_l, time, edge, L_ij, t, lambda_val, h, n, p, ground_truth, iter_gd, stepsize)
    ...
    return i, t, theta_h, ground_truth, W, sq_error
...
if __name__ == "__main__":
    ...
    seed = args.seed
    w_iter = args.w_iter
    iter_gd = args.iter_gd
    step_size = args.step_size

    T = np.linspace(0, 1, time_num)
    L_ij = np.full((n * n,), L_val)

    all_combinations = [(i, t, L_ij, p, n, lambda_val, h, seed, w_iter, iter_gd, step_size) for i in range(iter_num) for t in T]
...
#    with ProcessPoolExecutor(max_workers=args.cpu) as executor:
    ...
    MSE_error = np.zeros((iter_num, time_num, n, iter_gd))

    for i, t, theta_h, true_theta, W, sq_error in results:
        estimates[i, np.where(T == t)[0][0]] = theta_h
        ground_truth[i, np.where(T == t)[0][0]] = true_theta
        GMB_W[i, np.where(T == t)[0][0]] = W
        MSE_error[i, np.where(T == t)[0][0]] = sq_error

    result_dict = {'estimates': estimates, 'ground_truth': ground_truth, 'GMB_W': GMB_W, 'MSE_error': MSE_error}
    np.savez(f'iter{iter_num}_n{n}_Lij{L_val}_p{p}_h{h}_lambda{lambda_val}_gd{iter_gd}_step{step_size}.npz', **result_dict)